<template lang="pug">
    #Dict
        ...
        #main(v-if='item' :class='item.type' tabindex='2')
            h1#index(v-if='item.type=="JP_JTS"' v-html='item.index')
            #body(v-html="render(item)" @click='jump($event)')
...
<script lang="coffee">
    ...
    module.exports = 
        ...
        methods:
            ...
            select_item_i: (i)->
                @side.focus()
                @select_item(@items[i]) if 0 <= i < @items.length
            ...
            search: (index)->
                ...
                if @cache[index]
                    ...
                    url.search = new URLSearchParams index: @index
                    
                    fetch url
                        .then (resp)=> resp.json()
                        .then (retval)=>
                            if retval.err then console.log err
                            data = retval.data
                            data.forEach (x, i)-> x.i = i # 加入 index
                            if (@cache[@index] = @items = data)
                                @item = @items[0]
